/******************************************************************************
 * Image functions
 *   - Convert a 1bit framebuffer to GIF in RAM; based on giflib and gif_hash
 *   - PBM header
 *   - BMP header
 *
...
******************************************************************************/
void GifPutWord(std::vector<uint8_t>* gifBuffer, int Word) {
    unsigned char c[2];
    c[0] = LOBYTE(Word);
    c[1] = HIBYTE(Word);
...
size_t createBMP(uint8_t *frameBuffer, std::vector<uint8_t>* imageBuffer, uint16_t width, uint16_t height) {
...
size_t createPBM(uint8_t *frameBuffer, std::vector<uint8_t>* imageBuffer, uint16_t width, uint16_t height) {
...
size_t createGIF(uint8_t *framebuffer, std::vector<uint8_t>* gifBuffer, uint16_t width, uint16_t height) {
    ...
    if (gifprivate.HashTable) {
        free((char *)gifprivate.HashTable);
    }

    return gifBuffer->size();
}
